...
write_block(u_int blockno)
{
	...
	syscall_mem_map(0, va, 0, va, PTE_V);
...
alloc_block_num(void)
{
	...
	for(blockno = 3;blockno < super->s_nblocks; blockno++)
	{
		...
		{
			bitmap[blockno / 32] &= ~(1 << (blockno % 32));
			write_block(blockno/BIT2BLK);  //write the bitmap of this block back into disk
			return blockno;
		}
	}
	return -E_NO_DISK;
...
//the file here can either refer to a real file or a file directory, this function do not search child dir for the target
int
...
{
writef("f[j].f_name=%s,	f[j].f_size=%d , f[j].f_type= %d \n",f[j].f_name,f[j].f_size,f[j].f_type);
			if (strcmp(f[j].f_name, name) == 0) {
				*file = &f[j];
				f[j].f_dir = dir;  //valid only in mem
				return 0;
			}
}
	}
...
dir_alloc_file(struct File *dir, struct File **file)
{
	...
	dir->f_size += BY2BLK; //add one more block for file directory
	if ((r = file_get_block(dir, i, &blk)) < 0)  //in function file_map_block, a new block will be allocted
		return r;
	f = blk;
	*file = &f[0];
	return 0;		
...
writef("\ndir: %s dirtype %d \n",dir->f_name, dir->f_type);
		...
		name[path - p] = '\0';
		path = skip_slash(path);
      user_panic(" \n name: %s  %d \n", name,  dir_lookup(dir, name, &file));
		if (dir->f_type != FTYPE_DIR)
			return -E_NOT_FOUND;

		if ((r=dir_lookup(dir, name, &file)) < 0) {       // a directory was found but the file wasn't
...
//	return 0;
	int r;
	struct File *pdir,*f, *dum;
	char name[MAXNAMELEN];
       // writef("path : %s \n", path);
        file_create("/ ",dum);
          
        dum->f_size=10; dum->f_name[0]='s';dum->f_name[1]='d';dum->f_name[2]='\0';
        fs_sync(); 
