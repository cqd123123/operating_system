...
open_alloc(struct Open **o)
{
	...
	for (i = 0; i < MAXOPEN; i++) {
		...
		case 0:
			if ((r = syscall_mem_alloc(0, (u_int)opentab[i].o_ff, PTE_V)) < 0)
				return r;
...
{
	if (debug) writef("serve_open %08x %s 0x%x\n", envid, rq->req_path, rq->req_omode);
...
writef("serve_open:enter open %s\n",rq->req_path);
	// Find a file id.
	if ((r = open_alloc(&o)) < 0) {
		if (debug) writef("open_alloc failed: %e", r);
		goto out;
	}
	fileid = r;
//user_panic("serve_open:ending find a file id	o = %x\n",o);
	// Open the file.
	if ((r = file_open(path, &f)) < 0) {
		if (debug) writef("file_open failed: %e", r);
		goto out;
	}
...
writef("sending success, page %08x\n", (u_int)o->o_ff);
	ipc_send(envid, 0, (u_int)o->o_ff, PTE_V);
