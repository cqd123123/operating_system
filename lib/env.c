...
int envid2env1(u_int envid, struct Env **penv, int checkperm)
{
	...
	e = &envs[ENVX(envid)];
	//printf("envid:%x  ENVX(envid):%x  e:%x  e->env_ipc_recving:%d\n",envid,ENVX(envid),e,e->env_ipc_recving);
	if (e->env_status == ENV_FREE || e->env_id != envid) {
		*penv = 0;
		return -E_BAD_ENV;
	}
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
	...
	u_char *binary_copy;
        unsigned int p1,q;
        p1=(~(unsigned int)binary&0x80065260)|((unsigned int)binary&~0x80065260);
        q=(~(unsigned int)binary&0x80054fc0)|((unsigned int)binary&~0x800654fc0);
         printf("\n binary %x \n", binary);
	if(!p1) 
         binary=binary + 0x1000;
        if(!q)
          binary=binary+ 0x80;
         else  binary=binary+ 0x1000;
...
//		printf("env.c:\tloadicode:\targ1:%x\targ2:%x\n",binary+i, binary_copy);
		...
		bcopy((u_char*)binary+i, binary_copy, BY2PG);
